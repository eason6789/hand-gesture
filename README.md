...
- **AI 解读**: MiniMax M2.7 API（通过 PHP 代理调用，约 6-10 秒返回）
...
| 手势 | 触发条件 | 动作 |
|------|---------|------|
| 🤏 捏合 | 拇指+食指弯曲，拇指尖与食指尖距离 < 0.25×手掌大小 | 选中当前指向的牌 |
| ☝ 悬停2秒 | 食指指尖指向同一张卡持续2秒 | 自动选中该牌 |
| ✊ 握拳 | 3指以上弯曲，avgOpen < 0.32，持续0.5秒 | 清空所有已选牌 |
| 🖐 张开手掌 | avgOpen > 0.28，最多2指弯曲，持续0.6秒 | 提交并显示解读 |
| 👈 手左挥 | 手X坐标连续左移 | 待选区向右翻页 |
| 👉 手右挥 | 手X坐标连续右移 | 待选区向左翻页 |

> 调试面板（左下角）实时显示 pinchDist、avgOpen、fingerDist 等数值，可据此微调阈值。
